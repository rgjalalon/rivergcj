...
/** Slow drifting warm blob, the dark-mode cousin of the reference's gradient. */
const Blob: React.FC<{f: number; strength: number}> = ({f, strength}) => (
  <AbsoluteFill
    style={{
      opacity: strength,
      background: `radial-gradient(ellipse 45% 60% at ${38 + 10 * Math.sin(f / 70)}% ${62 + 8 * Math.cos(f / 90)}%, rgba(217,167,116,0.30) 0%, rgba(217,167,116,0.08) 45%, transparent 75%),
        radial-gradient(ellipse 40% 50% at ${72 + 6 * Math.cos(f / 60)}% ${30 + 6 * Math.sin(f / 80)}%, rgba(120,86,60,0.22) 0%, transparent 70%)`,
      filter: 'blur(40px)',
    }}
  />
);
...
const Caret: React.FC<{f: number; on?: boolean}> = ({f, on = true}) => (
  <span style={{display: 'inline-block', width: 3, height: '0.9em', marginLeft: 4, verticalAlign: '-0.08em', background: C.accent, opacity: on && Math.floor(f / 8) % 2 === 0 ? 1 : on ? 0.25 : 0}} />
);

const Cursor: React.FC<{x: number; y: number; press: number; opacity: number}> = ({x, y, press, opacity}) => (
  <svg width="44" height="52" viewBox="0 0 22 26" style={{position: 'absolute', left: x, top: y, opacity, transform: `scale(${1 - press * 0.15})`, transformOrigin: '4px 2px', filter: 'drop-shadow(0 6px 14px rgba(0,0,0,0.5))'}}>
    <path d="M2 1.5 L2 20 L7 15.5 L10.5 23.5 L13.8 22 L10.4 14.3 L17 14.3 Z" fill="#F3EEE7" stroke="#0B0A09" strokeWidth="1.3" strokeLinejoin="round" />
  </svg>
);
  ...
  return (
    <>
      ...
      <div style={{position: 'absolute', left: BAR.x, top: BAR.y + 96, fontFamily: fonts.sans, fontSize: 28, color: C.dim, opacity: 1 - p(f, B.split, 14), whiteSpace: 'nowrap'}}>
        ...
        <span style={{color: 'rgba(243,238,231,0.3)'}}>{typed(' · AMA, 2024, n=18,000', f, 62 + 30 / 1.6, 1.6)}</span>
        ...
        return (
          <div
            ...
            style={{
              ...
              background: C.card, border: `1px solid ${tick > 0 ? `rgba(217,167,116,${0.7 * tick})` : C.cardLine}`,
              boxShadow: `0 10px 30px rgba(0,0,0,0.45), 0 0 ${24 * tick}px rgba(217,167,116,${0.25 * tick})`,
    ...
    <AbsoluteFill style={{background: C.bg, overflow: 'hidden'}}>
      ...
      {stageOn && (
        <AbsoluteFill style={{perspective: 1400, opacity: (1 - close) * (f >= B.through ? p(f, B.through, 8) : 1)}}>
          ...
          <AbsoluteFill style={{alignItems: 'center', justifyContent: 'center', opacity: p(f, CLEARED + 38, 16)}}>
            <div style={{fontFamily: fonts.serif, fontSize: 118, color: '#1A120B', letterSpacing: -2, transform: `scale(${mix(1.08, 1, p(f, CLEARED + 38, 40))})`}}>
        ...
        <AbsoluteFill style={{alignItems: 'center', justifyContent: 'center', padding: '0 200px', opacity: p(f, B.line, 8) * (1 - p(f, B.prompt - 12, 12)), transform: `scale(${mix(1.06, 1, p(f, B.line, 80, Easing.out(Easing.quad)))})`}}>
          <div style={{fontFamily: fonts.serif, fontSize: 84, lineHeight: 1.16, color: C.text, letterSpacing: -1.2, textAlign: 'center'}}>
            {LINE.map((w, i) => {
              ...
              const hi = w === 'not';
              return (
                <span key={i} style={{display: 'inline-block', marginRight: '0.26em', opacity: t, transform: `translateY(${(1 - t) * 30}px)`, filter: `blur(${(1 - t) * 8}px)`, fontStyle: hi ? 'italic' : 'normal', color: hi ? C.accent : C.text}}>
                  {w}
                </span>
              );
            })}
      ...
      {promptOn && (
        <AbsoluteFill style={{alignItems: 'center', justifyContent: 'center', opacity: promptIn * (1 - p(f, B.through + 4, 10))}}>
          ...
          >
            <div style={{width: 14, height: 14, borderRadius: 7, background: C.accent, marginRight: 22, boxShadow: `0 0 ${12 + 6 * Math.sin(f / 4)}px rgba(217,167,116,0.8)`}} />
            <div style={{flex: 1, fontFamily: fonts.sans, fontSize: 40, color: C.text, letterSpacing: -0.4}}>
              {f < B.prompt + 10 ? <span style={{color: 'rgba(243,238,231,0.3)'}}>Ask Wai…</span> : typed(PROMPT, f, B.prompt + 10, 0.9)}
              {f >= B.prompt + 10 && <Caret f={f} on={f < B.click} />}
            </div>
            <div style={{width: 76, height: 76, borderRadius: 38, background: `linear-gradient(135deg, #E6B888, ${C.accent})`, display: 'flex', alignItems: 'center', justifyContent: 'center', transform: `scale(${1 - press * 0.12})`, boxShadow: `0 0 ${30 + 30 * press}px rgba(217,167,116,${0.35 + 0.4 * press})`}}>
              <svg width="34" height="34" viewBox="0 0 24 24"><path d="M5 12h13M12 5l7 7-7 7" stroke="#1A120B" strokeWidth="2.4" fill="none" strokeLinecap="round" strokeLinejoin="round" /></svg>
            </div>
          </div>
